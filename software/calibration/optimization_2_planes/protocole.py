...
def default_position(trial: dict):
    return position_from_imu_gravity(trial)
...
def zone_id_from_X(X: np.ndarray, n_bins: int = 6) -> np.ndarray:
    """
    Simple zoning:
      1) PCA -> 2D (via SVD)
      2) angular bins in [0..n_bins-1]

    Replace this with your analyse_space(...) if you already have per-sample zone_id.
    """
...
def cap_trials_per_zone_and_mass(
    trials: list[dict],
    X: np.ndarray,
    k_per_key: int,
    ...
    """
    Keep at most k_per_key trials per (zone_id, mass).

    - zone_id is computed from X if not provided.
    - This tends to preserve mass excitation while balancing geometric coverage.
    """
    ...
    if zone_id is None:
        zone_id = zone_id_from_X(X, n_bins=n_bins)

    zone_id = np.asarray(zone_id, dtype=int)
    if zone_id.shape[0] != len(trials):
        raise ValueError("zone_id must have shape (n_trials,) and match trials length")

    buckets: dict[tuple[int, Any], list[int]] = {}
    for i, t in enumerate(trials):
        key = (int(zone_id[i]), mass_fn(t))
...
# ---------------------------------------------------------------------
# Rewritten evaluate_protocol using cap_trials_per_zone_and_mass
# ---------------------------------------------------------------------
...
def evaluate_protocol(
    ...
    """
    Same structure as your original evaluate_protocol, but with an extra step:
    after select_trials, we optionally apply a balanced cap per (zone, mass).

    To enable it, add an attribute/field on ProtocolSpec, for example:
        spec.max_per_zone_mass = 2   # keep at most 2 per (zone, mass)

    If spec doesn't have that attribute, nothing changes.
    """
    # --- 1) select ---
    ...
    # --- 1b) OPTIONAL: cap per (zone, mass) BEFORE split ---
    k_zone_mass = getattr(spec, "max_per_zone_mass", None)
    if k_zone_mass is not None:
        # Build X for ALL chosen trials so zoning is coherent
        try:
            X_all, _, _ = build_XY(chosen, acc_bias, base)
        except TypeError:
            X_all, _, _ = build_XY(chosen, acc_bias, base)
        X_all = np.asarray(X_all, float)

        chosen = cap_trials_per_zone_and_mass(
            trials=list(chosen),
            X=X_all,
            k_per_key=int(k_zone_mass),
            mass_fn=mass_fn,
            seed=seed,
            n_bins=getattr(spec, "zone_bins", 6),
            zone_id=None,  # or your analyse_space(...)["zone_id"]
        )
    ...
    if spec.max_per_condition is not None:
        ...
        for t in selected:
            ...
            if counts[key] <= cap:
                kept.append(t)
        selected = kept
    print(f"Selected {len(selected)} trials for protocol '{spec.name}'")
    return selected
